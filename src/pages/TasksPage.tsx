...
const TasksPage = () => {
  ...
  // Tabs navigation
  type DashboardTab = 'templates' | 'assignments' | 'standalone' | 'standalone_kpis' | 'sales' | 'sales_kpis' | 'change_requests' | 'team' | 'daily_progress' | 'my_tasks';
  ...
  // Active assignment sub-filtering for agents/users / overdue / sales / manager
  const taskCategory: 'active' | 'overdue' | 'high' | 'done' = useMemo(() => {
    const rawStatus = searchParams.get('status');
    if (rawStatus && ['active', 'overdue', 'high', 'done'].includes(rawStatus)) {
      return rawStatus as 'active' | 'overdue' | 'high' | 'done';
    }
    return 'active';
  }, [searchParams]);

  const setTaskCategory = (newCategory: 'active' | 'overdue' | 'high' | 'done') => {
    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev);
  ...
  const renderMainContent = () => {
    ...
    return (
      <>
        ...
        {dashboardTab === 'my_tasks' && isElevated && viewMode === 'list' && (
          <UserDailyKpisView />
        )}

        {(dashboardTab === 'team' || dashboardTab === 'daily_progress') && isElevated && viewMode === 'list' && (
          <DailyTeamProgress />
        )}
        ...
        {(dashboardTab === 'standalone' || dashboardTab === 'standalone_kpis') && isElevated && viewMode === 'list' && (
          <Box>
